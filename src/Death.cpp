...
void TomatoSurvivor::death_menu()
{
    ...
    while (!WindowShouldClose() && waiting_for_input)
    {
        ...
        DrawText(TextFormat("Final Score: %.0f", getScore()), 280, 380, 30, BLACK);
        DrawText(TextFormat("Best Score: %d", SaveManager::getBestScore()), 250, 430, 30, BLACK);
        DrawText("Press ENTER to return to menu", 185, 520, 25, YELLOW);
        EndDrawing();
